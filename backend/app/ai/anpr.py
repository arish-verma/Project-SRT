...
class ANPREngine:
    ...
    def _read_plate(self,crop:np.ndarray):
        ...
        h,w=crop.shape[:2]
        regions=[]
        # Plate location varies by viewpoint: scan the lower 75%, center band and full vehicle.
        ...
        # Bright rectangular plate detector. This complements black-hat detection and works
        # particularly well for white Indian registration plates on dark/white cars.
        ...
        # Black-hat candidate detection for dark characters against a bright plate.
        ...
        # OCR the strongest candidates first, then fall back to the broad vehicle regions.
        seen=set()
    ...
    def _normalise_ocr(raw:str)->str:
        ...
        # OCR commonly confuses these glyphs. Apply conservative fixes only to improve
        # registration-number structure; never invent characters when there is no OCR text.
        text=text.replace(' ','')
        return text
    ...
    def _ocr_roi(self,roi):
        ...
        for scale in (4.,6.,8.):
            ...
            variants=[up,
                      ...
                      cv2.adaptiveThreshold(up,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,31,9)]
            for img in variants:
                for psm in (7,8,6,13):
