...
class VideoProcessor:
    ...
    def start(self, camera_id):
        ...
        with self._lock:
            ...
            self._workers[camera_id] = ProcessorState(
                threading.Thread(
                    target=self._run,
            ...
            self._workers[camera_id].thread.start()
            return True
    ...
    def is_running(self, camera_id):
        with self._lock:
            return bool((state := self._workers.get(camera_id)) and state.thread.is_alive())
        ...
        try:
            for packet in source.frames():
                if stop_event.is_set():
                    break
                frames += 1
                elapsed = max(__import__("time").monotonic() - started, .001)
    ...
    def process_frame(self, camera_id, packet):
        ...
        events.extend(
            self.event_engine.evaluate_drones(
                camera_id,
                self.pipeline.last_flying_detections.get(camera_id, []),
                zones,
                packet.frame.shape,
                packet.timestamp,
                scan_id=self.pipeline.last_flying_scan_frame.get(camera_id),
            )
        )
