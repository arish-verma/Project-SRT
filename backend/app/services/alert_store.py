...
class AlertStore:
    def __init__(self, db_path="storage/srt_events.db"):
        ...
        with sqlite3.connect(self.path) as db:
            db.execute("CREATE TABLE IF NOT EXISTS alerts (alert_id TEXT PRIMARY KEY,event_id TEXT NOT NULL UNIQUE,camera_id TEXT NOT NULL,created_at TEXT NOT NULL,severity TEXT NOT NULL,title TEXT NOT NULL,message TEXT NOT NULL,status TEXT NOT NULL)")
    ...
    def ensure_for_event(self, event: EventRecord):
        with self.lock, sqlite3.connect(self.path) as db:
            ...
            db.execute(
                ...
                (alert.alert_id, alert.event_id, alert.camera_id, alert.created_at.isoformat(),
                 alert.severity.value, alert.title, alert.message, alert.status.value),
            )
            return alert
    ...
    def _row(row):
        return AlertRecord(
            alert_id=row[0], event_id=row[1], camera_id=row[2],
            created_at=datetime.fromisoformat(row[3]), severity=row[4],
            title=row[5], message=row[6], status=row[7],
        )
    ...
    def delete(self, alert_id):
        with self.lock, sqlite3.connect(self.path) as db:
            if not db.execute("SELECT 1 FROM alerts WHERE alert_id=?", (alert_id,)).fetchone():
                return False
