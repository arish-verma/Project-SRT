...
class AlertStore:
    """Persistent alert repository backed by SQLite."""
    ...
    def ensure_for_event(self, event: EventRecord) -> AlertRecord:
        with self.lock, sqlite3.connect(self.path) as db:
            ...
            titles = {"INTRUSION":"Restricted Border Entered","NIGHT_MOVEMENT":"Night Movement Detected",
                      "LOITERING":"Loitering Detected","MULTI_PERSON_ACTIVITY":"Multiple-Person Activity Detected",
                      "ANOMALY_SUSPECTED":"Anomaly Detected","DRONE_DETECTED":"Drone Detected","ANPR_DETECTED":"ANPR Vehicle Detected"}
            alert = AlertRecord(alert_id=f"ALT-{uuid4().hex[:10].upper()}", event_id=event.event_id, camera_id=event.camera_id,
                created_at=datetime.now(timezone.utc), severity=event.severity, title=titles.get(event.event_type, f"{event.event_type.replace('_',' ').title()} Detected"), message=event.message)
            db.execute("INSERT INTO alerts VALUES (?,?,?,?,?,?,?,?)", (alert.alert_id,alert.event_id,alert.camera_id,alert.created_at.isoformat(),alert.severity.value,alert.title,alert.message,alert.status.value))
            return alert

    @staticmethod
    def _row(row) -> AlertRecord:
        return AlertRecord(alert_id=row[0],event_id=row[1],camera_id=row[2],created_at=datetime.fromisoformat(row[3]),severity=row[4],title=row[5],message=row[6],status=row[7])
    ...
    def delete(self, alert_id: str) -> bool:
        with self.lock, sqlite3.connect(self.path) as db:
            row=db.execute("SELECT 1 FROM alerts WHERE alert_id=?",(alert_id,)).fetchone()
