...
class RuleEventEngine:
 ...
 @staticmethod
 def _is_night(timestamp):h=datetime.fromtimestamp(timestamp).hour;return h>=22 or h<5
 @staticmethod
 ...
 def evaluate(self,camera_id,tracks,zones,frame_shape,timestamp=None):
  ...
  for zone in zones:
   ...
   for track in tracks:
    ...
    key=(camera_id,track.track_id,zone.zone_id);inside=self._contains_point(zone,*track.center,width,height)
    ...
    if entry and self._allow(key,now):
     risk=82 if zone.zone_type.value=='RESTRICTED' and track.label=='person' else 65;f=['restricted zone entry'] if zone.zone_type.value=='RESTRICTED' else ['monitored zone entry']
     if night and track.label=='person':risk=min(100,risk+10);f.append('night-time')
     if direction and direction!='STATIONARY':f.append(f'direction {direction.lower()}')
     sev=EventSeverity.CRITICAL if risk>=90 else EventSeverity.HIGH if risk>=75 else EventSeverity.MEDIUM;events.append(self._new_event(camera_id,now,'INTRUSION',sev,track.confidence,risk,f'{track.label.title()} entered {zone.name}',track.label,track.track_id,zone,{'zone_type':zone.zone_type.value,'factors':f,'night':night,'dwell_seconds':round(dwell,1),'direction':direction}))
 ...
 def _assign_drone_tracks(self,camera_id,detections):
  prev=self._drone_tracks[camera_id];out=[];used=set()
  for d in detections:
   best=None;score=.2
