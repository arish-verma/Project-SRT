...
class RuleEventEngine:
    ...
    def _contains_point(zone, x, y, width, height):
        polygon = __import__("numpy").array(
            [(px * width, py * height) for px, py in zone.polygon], dtype="float32"
        )
    ...
    def _allow(self, key, now, cooldown=None):
        interval = cooldown if cooldown is not None else self.cooldown_seconds
        if now - self._last_event.get(key, 0) < interval:
            return False
    ...
    def evaluate(self, camera_id, tracks, zones, frame_shape, timestamp=None):
        ...
        persons = [track for track in tracks if track.label == "person"]
                ...
                if entry and self._allow(key, now):
                    ...
                    events.append(
                        self._new_event(
                            ...
                            track.confidence,
                            risk,
                            f"{track.label.title()} entered {zone.name}",
                            track.label,
                            track.track_id,
                            zone,
                            {"zone_type": zone.zone_type.value, "factors": factors, "night": night, "dwell_seconds": round(dwell, 1), "direction": direction},
                        )
                    )
                if track.label == "person" and zone.zone_type.value == "RESTRICTED":
                    if dwell >= 30 and self._allow((camera_id, track.track_id, zone.zone_id, "loiter"), now, 15) and movement < max(width, height) * 0.18:
                    ...
                    if len(history) >= 8 and movement > max(width, height) * 0.75 and self._allow((camera_id, track.track_id, zone.zone_id, "anomaly"), now, 20):
                        events.append(self._new_event(camera_id, now, "ANOMALY_SUSPECTED", EventSeverity.HIGH, track.confidence, 78, f"Unusual movement pattern detected in {zone.name}", "person", track.track_id, zone, {"factors": ["unusual movement speed", "restricted zone"], "movement_pixels": round(movement, 1), "direction": direction, "note": "Operator-assistance signal, not proof of intent."}))
            if persons_inside >= 3 and self._allow((camera_id, zone.zone_id, "group"), now, 20):
                events.append(self._new_event(camera_id, now, "MULTI_PERSON_ACTIVITY", EventSeverity.HIGH, 0.9, 78, f"Multiple people detected in {zone.name}", "person", None, zone, {"person_count": persons_inside, "factors": ["restricted zone", "multiple people"]}))
    ...
    def _assign_drone_tracks(self, camera_id, detections):
        ...
        self._drone_tracks[camera_id] = {track_id: detection.bbox for track_id, detection in output}
        return output

    def evaluate_drones(self, camera_id, detections, zones, frame_shape, timestamp=None):
        """Create security alerts only for a confirmed drone class.

        Airplanes and helicopters remain visible as classified detections but do
        not enter the alert/event pipeline. This is an intentional product rule.
        """
        now = timestamp or time.time()
        height, width = frame_shape[:2]
        events = []
        ...
        drones = [
            detection
            for detection in detections
            if detection.label.lower() == "drone" and detection.confidence >= settings.drone_model_confidence
        ]
        if not drones:
            return events

        # One alert per camera/cooldown window avoids an alarm storm when the
        # same drone is represented by several nearby detections/tracks.
        if not self._allow((camera_id, "drone_alert"), now, settings.drone_alert_cooldown_seconds):
            return events
        ...
        for track_id, detection in self._assign_drone_tracks(camera_id, drones):
            ...
            inside = bool(
                restricted
                and self._contains_point(restricted, (x1 + x2) / 2, (y1 + y2) / 2, width, height)
            )
            risk = 95 if inside else 82
            factors = ["confirmed drone classification"]
            if inside:
                factors.append("restricted zone")
            message = f"Drone detected in camera {camera_id} at {datetime.fromtimestamp(now, tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}"
            if inside:
            ...
            events.append(
                self._new_event(
                    ...
                    now,
                    "DRONE_DETECTED",
                    EventSeverity.CRITICAL if inside else EventSeverity.HIGH,
                    ...
                    {
                        "aerial_class": "drone",
                        "aerial_track_id": track_id,
                        "bbox": list(detection.bbox),
                        "camera_id": camera_id,
                        "detected_at": datetime.fromtimestamp(now, tz=timezone.utc).isoformat(),
                        "factors": factors,
                        "note": "Alert is emitted only after temporal confirmation of the drone class; operator review recommended.",
                    },
                )
            )
        return events
