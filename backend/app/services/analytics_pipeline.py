...
class AnalyticsPipeline:
    ...
    def _merge_anpr(self, camera_id: str, new: list[dict]) -> list[dict]:
        ...
        merged = []
        for item in new:
            best = max((p for p in old if p.get("vehicle_type") == item.get("vehicle_type")),
                       key=lambda p: self._iou(item["bbox"], p.get("bbox", [0, 0, 0, 0])), default=None)
    ...
    def _annotate_and_store(self, camera_id: str, frame: Any, frame_index: int, timestamp: float, detections, tracks) -> None:
        ...
        if ok:
            self.frame_store.put(camera_id, FrameSnapshot(jpeg=encoded.tobytes(), frame_index=frame_index,
    ...
    def process(self, camera_id: str, packet: Any):
        ...
        # Always publish a frame before optional heavy specialist work.
        ...
        if packet.frame_index % self.anpr_scan_interval == 0:
            try:
                self.last_anpr[camera_id] = self._merge_anpr(camera_id, self.anpr.scan(frame, detections, camera_id=camera_id, timestamp=packet.timestamp))
            except Exception:
