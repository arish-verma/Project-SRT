...
class AnalyticsPipeline:
 def __init__(self,frame_store:FrameStore,model_path:str='yolo11n.pt',confidence:float=.35)->None:
  self.frame_store=frame_store;self.model_path=model_path;self.confidence=confidence;self.detector=YOLODetector(model_path,confidence);self._trackers={};self._tracker_lock=threading.RLock();self._inference_lock=threading.RLock();self.drone_detector=DroneDetector(settings.drone_model_path,settings.drone_model_confidence);self.drone_scan_interval=max(1,settings.drone_scan_interval);self.drone_enabled=False;self.last_drone_detections=[];self.last_detections={};self.last_tracks={};self.last_anpr={};self.anpr=ANPREngine()
 ...
 def process(self,camera_id:str,packet:Any):
  ...
  # OCR is deliberately throttled so many concurrent CCTV channels remain responsive.
