...
class AnalyticsPipeline:
 ...
 def process(self,camera_id:str,packet:Any):
  ...
  if self.drone_enabled and packet.frame_index%self.drone_scan_interval==0:
   ...
   except Exception:logger.exception('Drone inference failed for %s',camera_id);self.last_drone_detections=[]
  ...
  for p in self.last_anpr.get(camera_id,[]):
   bx=p.get('plate_bbox') or p.get('bbox');x1,y1,x2,y2=map(int,bx)
   cv2.rectangle(annotated,(x1,y1),(x2,y2),(0,255,255),2)
   label=f"PLATE {p['plate_text']}" if p.get('plate_text') else 'PLATE CANDIDATE'
