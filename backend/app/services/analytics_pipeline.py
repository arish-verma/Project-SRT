...
class AnalyticsPipeline:
    def __init__(self,frame_store:FrameStore,model_path:str='yolo11n.pt',confidence:float=.35)->None:
        self.frame_store=frame_store;self.model_path=model_path;self.confidence=confidence;self.detector=YOLODetector(model_path,confidence);self._trackers={};self._tracker_lock=threading.RLock();self._inference_lock=threading.RLock();self.drone_detector=DroneDetector(settings.drone_model_path,settings.drone_model_confidence);self.drone_scan_interval=max(1,settings.drone_scan_interval);self.drone_enabled=True;self.last_drone_detections=[];self.last_detections={};self.last_tracks={};self.last_anpr={};self.last_faces={};self.anpr=ANPREngine();self.face_service=FaceDetectionService()
    ...
    def process(self,camera_id:str,packet:Any):
        ...
        if self.drone_enabled and packet.frame_index%self.drone_scan_interval==0:
            try:
                # The base model may already identify an airplane/aircraft/helicopter.
                # The specialist model supplements it for small drones. Everything is
                # normalized to one operator-facing class: AERIAL OBJECT.
                ...
                combined=[]
                for d in aerial:combined.append(Detection(label='aerial_object',confidence=d.confidence,bbox=d.bbox))
                combined.extend(self.last_drone_detections)
                self.last_drone_detections=self._dedupe_aerial(combined)
            except Exception:logger.exception('Aerial inference failed for %s',camera_id);self.last_drone_detections=[]
        annotated=frame.copy()
        for d in detections:
            # Do not show the raw aerial class from the generic detector.
        ...
        for p in self.last_anpr.get(camera_id,[]):
            bx=p.get('plate_bbox') or p.get('bbox');x1,y1,x2,y2=map(int,bx);cv2.rectangle(annotated,(x1,y1),(x2,y2),(0,255,255),2)
            label=f"PLATE {p['plate_text']}" if p.get('plate_text') else 'PLATE CANDIDATE';cv2.putText(annotated,label,(x1,max(18,y1-6)),cv2.FONT_HERSHEY_SIMPLEX,.55,(0,255,255),2,cv2.LINE_AA)
