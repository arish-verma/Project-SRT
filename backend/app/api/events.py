...
def recording(event_id:str):
 e=next((x for x in event_store.list(limit=500) if x.event_id==event_id),None);p=Path((e.metadata or {}).get('recording_clip','')) if e else Path('')
...
def clear_events(): event_store.clear()
